...
class SpellEffect(object):
	...
	def rollSpell(self, researchlevel, forceschool=None, forcepath=None, isnextspell=False, forcesecondaryeff=None, blockmodifier=False, blocksecondary=False, allowblood=True, allowskipchance=True, setparams=None, forcedmodifier=None, forcepathlevel=None, **options):
		...
		if forceschool and not (self.schools & forceschool):
			print(f"Failed to generate {self.name} at {researchlevel}: school is forced to {forceschool} which is incompatible")
			return None
		...
		if researchlevel + mod.power + secondary.power < self.power and self.paths != 256 and not (isnextspell and self.scalerate == 0):
			#print("bad power")
			print(f"Failed to generate {self.name} at {researchlevel}: final power level too low")
			return None
		
		if researchlevel + mod.power + secondary.power > self.maxpower  and self.paths != 256 and not (isnextspell and self.scalerate == 0):
		...
		if self.chassisvalue is not None:
			...
			if self.magicvalue < 0.0:
				raise ValueError(f"Can't have a negative (fatiguecost-chassisvalue) of {self.magicvalue} for {self.name}")
		...
		if self.spelltype & SpellTypes.POWER_SCALES_AOE or self.spelltype & SpellTypes.POWER_SCALES_DAMAGE or self.spelltype & SpellTypes.POWER_SCALES_NREFF or self.spelltype & SpellTypes.POWER_SCALES_MAXBOUNCES or self.spelltype & SpellTypes.POWER_SCALES_EFFECTNO:
			...
			print(f"Scaling path calc is {mod.pathperresearch+self.pathperresearch+secondary.pathperresearch} * {actualpowerlvl}")
			print(f"Scaling path level modification is {math.floor((mod.pathperresearch+self.pathperresearch+secondary.pathperresearch)*actualpowerlvl)}")
			s.path1level += math.floor((mod.pathperresearch+self.pathperresearch+secondary.pathperresearch)*actualpowerlvl)
		...
		if self.nextspell != "":
			s.nextspell = self.nextspell.rollSpell(researchlevel + mod.power + secondary.power, forceschool=forceschool, forcepath=s.path1, blockmodifier=True, isnextspell=True, setparams=setparams, allowskipchance=False)
		if self.extraspell != "":
			extraspell = utils.spelleffects[self.extraspell]
			if self.donotsetextraspellpath == 0:
				s.modcmdsbefore += extraspell.rollSpell(researchlevel, forceschool=s.school, forcepath=s.path1, isnextspell=True, blockmodifier=False, setparams=setparams, forcedmodifier=mod, forcepathlevel=s.path1level, allowskipchance=False).output()
			else:
				s.modcmdsbefore += extraspell.rollSpell(researchlevel, forceschool=s.school, isnextspell=True, blockmodifier=False, setparams=setparams, forcedmodifier=mod, forcepathlevel=s.path1level, allowskipchance=False).output()
		...
		if s.path1level < 1 and self.pathlevel > 0:
			...
			return self.rollSpell(researchlevel, forceschool=forceschool, forcepath=forcepath, isnextspell=isnextspell, forcesecondaryeff=forcesecondaryeff, blocksecondary=blocksecondary, allowblood=allowblood, allowskipchance=allowskipchance, setparams=setparams, forcepathlevel=forcepathlevel, forcedmodifier=forcedmodifier)
		...
		for attrib, val in secondary.multcommands:
			...
			if attrib == "fatiguecost" and self.chassisvalue is not None:
				print(f"magicvalpercent {self.magicvaluepercent}, magicpathvaluescaling {secondary.magicpathvaluescaling}, chassisvaluepercent {self.chassisvaluepercent}")
				...
				newval = int(s.fatiguecost * (self.magicvaluepercent + self.chassisvaluepercent) + magiccostmod + chassiscostmod)
				print(f"secondary.magicpathvaluescaling = {secondary.magicpathvaluescaling}: base magic: {self.magicvalue}, magic mod: {magiccostmod}, base chassis: {self.chassisvalue}, chassis mod: {chassiscostmod}, final={newval}, effectfatigue={self.fatiguecost}, oldspellfatigue={s.fatiguecost}")
			else:
				newval = int(getattr(s, attrib) * val)
					...
					if secondary.nextspell != "":
						...
						while 1:
							if isinstance(tmp.nextspell, Spell):
								...
								tmp.nextspell = utils.spelleffects[secondary.nextspell].rollSpell(researchlevel, forcepath=s.path1, isnextspell=True, blockmodifier=True, setparams=setparams)
		...
		if len(moddescrs) > 0:
			s.descr += naming.parsestring(random.choice(moddescrs), plural=plural, aoe=s.aoe, spelltype=self.spelltype, spell=s)
		...
		if len(names) > 0:
			while 1:
				try:
					...
					s.name = naming.parsestring(name, plural=plural, aoe=s.aoe, spelltype=self.spelltype, titlecase=True, isspell=True)
					break
