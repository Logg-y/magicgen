...
class WeaponMod(object):
    ...
    def _applytoweapon(self, wpn, replacements):
        out = ""
        # Do Nothing can safely skip vanilla weapons
        if self.name == "Do Nothing" and not wpn.isnewweapon:
            return ""
        out += f"-- Modified weapon {wpn.uniqueid} with weaponmod {self.name}\n"
        ...
        if utils.WEAPON_ID >= 2000:
            raise ValueError("Modded weapon ID is too high - consider reducing the number of modified"
                             " creatures generated!")
        replacements[wpn.uniqueid] = copy(utils.WEAPON_ID)
        utils.WEAPON_ID += 1
        if wpn.origid is not None:
            out += f"#copyweapon {wpn.origid}\n"
        spec = copy(wpn.spec)

        out += wpn.additionalmodcmds + "\n"
        ...
        for param, val in self.setcommands:
            modcmd = flags_to_mod_cmds.get(param, param)
            if param in ["secondaryeffectalwaysid", "secondaryeffectid"] and self.secondaryeffectnewweapon is not None:
                newwpn = utils.newweapons[self.secondaryeffectnewweapon].toWeapon()
                secondaryeffectcode = utils.weaponmods["Do Nothing"]._applytoweapon(newwpn, replacements)
                out = secondaryeffectcode + out
                val = replacements[newwpn.uniqueid]
