...
class SpellEffect(object):
    def __init__(self, fp=None):
        self.fp = fp
        # Basic parameters, schools/paths are bitmask-ified though
        self.name = None
        self.effect = -1
        self.damage = -1
        self.spec = 0
        ...
        self.copyspell = None
        # How much to make spell pathlevels increase with research
        self.pathperresearch = 0.66
        self.casttime = None
        self.provrange = None
        # Chance to gain a crosspath
        ...
        # When copying vanilla spells, these allow omitting the lines that normally overwrite sprites
        ...
        self.onlygeosrc = 0
        # Can only generate once, and never nationally
        self.unique = 0
        # How many times this effect has generated this set
        self.generated = 0
        # The non-national Spell objects generated by this effect
        self.genericchildspells: List[Spell] = []
        # Whether or not this should be always get generated at the end if it didn't get rolled naturally
        ...
        self.scaledoubletime = {}
        self.scalelinear = {}
        self.attribscalingperpathlevels = {}
    ...
    def rollSpell(self, researchlevel, forceschool=None, forcepath=None, isnextspell=False, forcesecondaryeff=None,
        ...
        scaleamt = self.calcScaleamt(mod, secondary, actualpowerlvl, None)
        ...
        s.effectiveaoe = utils.unpackIllwinterScaling(s.aoe, s.path1level)
        s.effectivenreff = utils.unpackIllwinterScaling(s.nreff, s.path1level)
        # This one will be wrong for nondamaging spells, but shouldn't be used for them
        s.effectivedamage = utils.unpackIllwinterScaling(s.damage, s.path1level)
    ...
    def enforceGemCosts(self, s):
        ...
        if s.effect == 8 and self.fatiguecost >= 100:  # reinvigoration
            ...
            if s.fatiguecost < 100:
                ...
                scalenreff = utils.unpackIllwinterScaling(s.nreff, s.path1level)
    ...
    def calcAISpellMod(self, spell):
        ...
        realaoe = utils.unpackIllwinterScaling(spell.aoe, spell.path1level)
        ...
        realnreff = utils.unpackIllwinterScaling(spell.nreff, spell.path1level)
    ...
    def scaleSpellEffectsToFatigue(self, s, desiredfatigue):
        ...
        realnumeffects = utils.unpackIllwinterScaling(s.nreff, s.path1level)
        realaoe = utils.unpackIllwinterScaling(s.aoe, s.path1level)
    ...
    def _scaleSpellEffectiveness(self, s, actualpowerlvl, mod, secondary):
        ...
        if self.spelltype & SpellTypes.POWER_SCALES_DAMAGE:
            ...
            s.damage = scaleamt
            print(f"scaled damage: {self.damage} ->  {s.damage}")
        if self.spelltype & SpellTypes.POWER_SCALES_NREFF:
            scaleamt = self.calcScaleamt(mod, secondary, actualpowerlvl, "nreff")
            s.nreff = scaleamt
            print(f"scaled nreff: {self.nreff} ->  {s.nreff}")
    ...
    def calculatedExpectedFinalFatigueAtPowerLevel(self, actualpowerlvl, modifier, secondary):
        ...
        finalfatigue += self._calcFatigueFromAdditionalEffects(actualpowerlvl)
    ...
    def _powerLevelIsValid(self, mod, secondary):
        ...
        if self.researchlevel + mod.power + secondarypower < self.power and self.paths != 256 and not self.isnextspell:
            print(f"Failed to generate {self.name} at {self.researchlevel}: final power level too low")
            return False

        if self.researchlevel + mod.power + secondarypower > self.maxpower and self.paths != 256 and not self.isnextspell:
    ...
    def _scaleFatigueCost(self, s, mod, secondary, **options):
        ...
        realnumeffects = utils.unpackIllwinterScaling(s.nreff, s.path1level)
        s.fatiguecost += self._calcFatigueFromAdditionalEffects(actualpowerlvl)
    ...
    def formatDetails(self, s, mod, secondary):
        ...
        actualpowerlvl = self.calcActualpowerlvl(mod, secondary)
        scaleamt = self.calcScaleamt(mod, secondary, actualpowerlvl, None)
        finalglobalscaling = int(scaleamt)
