...
def rollspells(**options):
	with open("log.txt", "w") as logfile:
		...
		with open(f"magicgen-{modname}.dm", "w") as f:
			...
			readModifiersFromDir(r".\data\spells\modifiers")
			readSecondariesFromDir(r".\data\spells\secondaries")
			readSecondariesFromDir(r".\data\spells\secondaries\summons")
			readUnitModsFromDir(r".\data\spells\secondaries\summons\unitmods")
			readWeaponModsFromDir(r".\data\spells\secondaries\summons\unitmods\weaponmods")
			# dict merging
			# (or I could upgrade to py3.9 to use |=)
			s = readEffectsFromDir(r".\data\spells\secondaries\nextspells")
			s = {**s, **readEffectsFromDir(r".\data\spells\summons")}
			s = {**s, **readEffectsFromDir(r".\data\spells\summons\commanders")}
			s = {**s, **readEffectsFromDir(r".\data\spells\rituals")}
			
			s = {**s, **readEffectsFromDir(r".\data\spells")}
			...
			for school in [1, 2, 4, 8, 16, 32, 64]:
				...
				for research in range(0, 10):
					if school == 8 and research in [2, 4, 6, 8]:
						...
						continue
					sys.stderr.write(f"Generating {spellsperlevel} spells at research {research} for school {schoolname}...\n")
					effectpool = copy.copy(s)
					for x in range(0, spellsperlevel):
						while 1:
							spell = None
							if len(effectpool) == 0:
								print(f"WARNING: no valid spells at research {research} for school {schoolname}, generated {x}/{spellsperlevel} successfully")
								sys.stderr.write(f"No more valid spells at research {research} for school {schoolname}, generated {x}/{spellsperlevel} successfully\n")
								break
							...
							if spell is not None:
								...
								print(f"Successfully generated spell {spell.name} from effect {sp.name} at research level {research}")
								break
						if len(effectpool) == 0:
							break
			...
			for name, spelleff in s.items():
				if spelleff.alwaysgenerate > 0 and not spelleff.generated:
					...
					spell = spelleff.rollSpell(random.randint(spelleff.power, spelleff.maxpower), allowskipchance=False, **options)
					if spell is not None:
						l.append(spell)
			...
			# Make holy spells
			holy = readEffectsFromDir(r".\data\spells\holy")
			for name, spelleff in holy.items():
				for path in [1, 2, 4, 8, 16, 32, 64, 128]:
					l.append(spelleff.rollSpell(spelleff.power, forcesecondaryeff=path, blockmodifier=True, allowskipchance=False, **options))
			...
			for index, nation in enumerate(list(nationals.nationals.keys())):
				...
				for x in range(0, options.get("nationalspells", 12)):
					...
					while 1:
						...
						if (choseneffect.name not in successfuleffectnames) and (choseneffect.name not in generatedeffectsatlevels[researchlevel]):
							...
							if len(allowedpaths) > 0:
								...
								if primarypath is not None:
									...
									print(f"Try generating national spell for {nation} with effect {choseneffect.name}, primarypath={primarypath}, secondaries={chosencomm.allpaths}, there are {len(effectpool)} effects")
									for attempt in range(0, 2):
										if attempt == 0:
											spell = choseneffect.rollSpell(researchlevel, forcepath=primarypath, forcesecondaryeff=chosencomm.allpaths, allowblood=allowblood, allowskipchance=False, setparams={"restricted":nation}, **options)
										else:
											spell = choseneffect.rollSpell(researchlevel, forcepath=primarypath, blocksecondary=True, allowblood=allowblood, allowskipchance=False, setparams={"restricted":nation}, **options)
										if spell is not None:
											l.append(spell)
											succeeded = True
						...
						if len(effectpool) == 0:
							if len(researchlevelstotry) == 0:
								raise ValueError(f"Couldn't make a national spell for nation {nation}, guaranteed={chosencomm.guaranteedpaths}, randoms={chosencomm.allpaths}")
							researchlevel = researchlevelstotry.pop(0)
							effectpool = copy.copy(s)
...
def main():
	opts = []
	opts.append(Option("-spellsperlevel", help="Number of spells to try making at each research level", type=int, default=14))
	opts.append(Option("-constructionfactor", help="Construction will get only this proportion of the normal number of spells. This is intended to be less than 1.0.", type=float, default=0.33))
	opts.append(Option("-modlist", help="A list of .dm file paths, separated by commas. Files in the list will be scanned and nations defined in them will get national spells.", type=str, default=""))
	opts.append(Option("-nationalspells", help="Number of national spells to try to make per nation. These spells will be directed towards the paths the nation has access to.", type=int, default=12))
	opts.append(Option("-modname", help="Name of the mod. If left blank a rather unhelpful number will be generated at random.", default=None))

	if len(sys.argv) > 1:
		parser = argparse.ArgumentParser(prog=f"MagicGen v{ver}", description="Procedural generator for Dom5 spellbooks!",
